...
function CalculateBlock(adjusted, size, multipliers, round = 'floor', offset = 0, baseScore = 0) {
    let score = 0;
...
function UniqueSkillScore(unique_skill_level, uma_star_level) {
    const multiplier = uma_star_level > 2 ? 170 : 120;
    return unique_skill_level * multiplier;
}
export async function rating_calculator(input) {
    const raw = { skills: {} };
    ...
    const { rating, next_rank } = Rating(total_score);
    return {
        total_score,
        rating,
        next_rank,
